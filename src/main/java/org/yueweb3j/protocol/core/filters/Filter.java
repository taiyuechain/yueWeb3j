...
import org.yueweb3j.protocol.core.Response.Error;
import org.yueweb3j.protocol.core.RpcErrors;
import org.yueweb3j.protocol.core.methods.response.YueFilter;
import org.yueweb3j.protocol.core.methods.response.YueLog;
import org.yueweb3j.protocol.core.methods.response.YueUninstallFilter;
...
public abstract class Filter<T> {
    ...
    public void run(ScheduledExecutorService scheduledExecutorService, long blockTime) {
        try {
            YueFilter yueFilter = sendRequest();
            if (yueFilter.hasError()) {
                throwException(yueFilter.getError());
            }

            filterId = yueFilter.getFilterId();
    ...
    private void getInitialFilterLogs() {
        try {
            Optional<Request<?, YueLog>> maybeRequest = this.getFilterLogs(this.filterId);
            YueLog yueLog = null;
            if (maybeRequest.isPresent()) {
                yueLog = maybeRequest.get().send();
            } else {
                yueLog = new YueLog();
    ...
    private void pollFilter(YueFilter yueFilter) {
        YueLog yueLog = null;
        try {
            yueLog = web3j.yueGetFilterChanges(filterId).send();
        } catch (IOException e) {
            throwException(e);
        }
        if (yueLog.hasError()) {
            Error error = yueLog.getError();
    ...
    public void cancel() {
        ...
        try {
            ...
            if (!yueUninstallFilter.isUninstalled()) {
     ...
     */
    protected abstract Optional<Request<?, YueLog>> getFilterLogs(BigInteger filterId);
