 ...
 */
package org.yueweb3j.protocol.rx;
...
import org.yueweb3j.utils.Flowables;
...
public class JsonRpc2_0Rx {
    ...
    public Flowable<String> yueBlockHashFlowable(long pollingInterval) {
    ...
    public Flowable<String> yuePendingTransactionHashFlowable(long pollingInterval) {
        return Flowable.create(
                subscriber -> {
    ...
    private <T> void run(
            org.yueweb3j.protocol.core.filters.Filter<T> filter,
    ...
    public Flowable<Transaction> pendingTransactionFlowable(long pollingInterval) {
        return yuePendingTransactionHashFlowable(pollingInterval)
                .flatMap(
                        transactionHash ->
                                web3j.yueGetTransactionByHash(transactionHash).flowable())
                .filter(yueTransaction -> yueTransaction.getTransaction().isPresent())
                .map(yueTransaction -> yueTransaction.getTransaction().get());
    }

    public Flowable<YueBlock> blockFlowable(boolean fullTransactionObjects, long pollingInterval) {
        return yueBlockHashFlowable(pollingInterval)
                .flatMap(
                        blockHash ->
                                web3j.yueGetBlockByHash(blockHash, fullTransactionObjects)
                                        .flowable());
    }

    public Flowable<YueBlock> replayBlocksFlowable(
    ...
    private Flowable<YueBlock> replayPastBlocksFlowableSync(
            DefaultBlockParameter startBlock,
            boolean fullTransactionObjects,
            Flowable<YueBlock> onCompleteFlowable) {
    ...
    private BigInteger getBlockNumber(DefaultBlockParameter defaultBlockParameter)
        ...
        if (defaultBlockParameter instanceof DefaultBlockParameterNumber) {
            ...
            YueBlock latestYueBlock =
                    web3j.yueGetBlockByNumber(defaultBlockParameter, false).send();
            return latestYueBlock.getBlock().getNumber();
        }
    }

    private static List<Transaction> toTransactions(YueBlock yueBlock) {
        ...
        return yueBlock.getBlock().getTransactions().stream()
