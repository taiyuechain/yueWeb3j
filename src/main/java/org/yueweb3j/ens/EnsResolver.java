 ...
 */
package org.yueweb3j.ens;

import org.yueweb3j.crypto.Keys;
import org.yueweb3j.crypto.WalletUtils;
import org.yueweb3j.ens.contracts.generated.ENS;
import org.yueweb3j.ens.contracts.generated.PublicResolver;
...
import org.yueweb3j.protocol.core.methods.response.YueSyncing;
import org.yueweb3j.protocol.core.methods.response.NetVersion;
import org.yueweb3j.tx.ClientTransactionManager;
import org.yueweb3j.tx.TransactionManager;
import org.yueweb3j.tx.gas.DefaultGasProvider;
...
public class EnsResolver {
    ...
    boolean isSynced() throws Exception {
        YueSyncing yueSyncing = web3j.yueSyncing().send();
        if (yueSyncing.isSyncing()) {
            return false;
        } else {
            YueBlock yueBlock =
                    web3j.yueGetBlockByNumber(DefaultBlockParameterName.LATEST, false).send();
            long timestamp = yueBlock.getBlock().getTimestamp().longValueExact() * 1000;
